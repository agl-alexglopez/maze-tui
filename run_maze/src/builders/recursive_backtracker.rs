...
pub fn generate_recursive_backtracker_maze(maze: &mut maze::Maze) {
    ...
    let mut random_direction_indices: Vec<usize> = (0..maze_util::NUM_DIRECTIONS).collect();
    ...
    while branches_remain {
        ...
        for i in &random_direction_indices {
            let direction: &maze::Point = &maze_util::GENERATE_DIRECTIONS[*i];
            let next = maze::Point {
        ...
        if !branches_remain && cur != start {
            let dir: maze_util::BacktrackMarker = (maze[cur.row as usize][cur.col as usize]
                & maze_util::MARKERS_MASK)
                >> maze_util::MARKER_SHIFT;
            // The solvers will need these bits later so we need to clear bits.
            maze[cur.row as usize][cur.col as usize] &= !maze_util::MARKERS_MASK;
            let backtracking: &maze::Point = &maze_util::BACKTRACKING_POINTS[dir as usize];
...
pub fn animate_recursive_backtracker_maze(maze: &mut maze::Maze, speed: maze_util::BuilderSpeed) {
    ...
    while branches_remain {
        ...
        branches_remain = false;
        for i in &random_direction_indices {
            let direction = &maze_util::GENERATE_DIRECTIONS[*i];
            let next = maze::Point {
                row: cur.row + direction.row,
                col: cur.col + direction.col,
            };
