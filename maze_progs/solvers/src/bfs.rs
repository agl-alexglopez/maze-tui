    ...
    let mut all_starts: [maze::Point; 4] = if let Ok(mut lk) = monitor.lock() {
        ...
        for p in maze::ALL_DIRECTIONS {
            ...
            lk.maze.solve_history.push(tape::Delta {
                id: next,
                before: next_square,
                after: next_square | maze::PATH_BIT,
                burst: BURST,
            });
        ...
        lk.maze.solve_history.push(tape::Delta {
            id: finish,
            before: finish_square,
            after: finish_square | solve::FINISH_BIT | maze::PATH_BIT,
            burst: BURST,
        });
        *lk.maze.get_mut(finish.row, finish.col) |= solve::FINISH_BIT | maze::PATH_BIT;
        all_starts
    } else {
        print::maze_panic!("Thread panick.");
    };
...
fn gatherer_history(monitor: monitor::MazeMonitor, guide: solve::ThreadGuide) {
    ...
    while let Some(cur) = bfs.pop_front() {
        if let Ok(mut lk) = monitor.lock() {
            ...
            let finish = (before & solve::FINISH_BIT) != 0;
            let first = (before & solve::CACHE_MASK) == 0;
            ...
            match (finish, first) {
                (true, true) => {
                    lk.maze.solve_history.push(tape::Delta {
                        id: cur,
                        before,
                        after: before | guide.paint | seen_bit,
                        burst: BURST,
                    });
                ...
                _ => {
                    lk.maze.solve_history.push(tape::Delta {
                        id: cur,
                        before,
                        after: before | seen_bit | guide.paint,
                        burst: BURST,
                    });
